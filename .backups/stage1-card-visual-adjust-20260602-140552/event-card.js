...
(function (window) {
    ...
    function renderEventListCard(event) {
        ...
        const hasMetric = event.metricName || event.impact;
        ...
        return `
            <article class="event-card ${priorityClass} ${isPinned} ${hasMetric ? 'has-metric-link' : ''}"
                ...
                <div class="event-card-top">
                    <div class="event-source-icon source-neutral">
                        ${ui.icon(sourceIcon)}
                    </div>
                ...
                ${hasMetric ? `
                <div class="event-card-metric-link">
                    <div class="event-card-metric-main">
                        ${ui.icon('bar-chart-3')}
                        <span>${ui.escapeHtml(event.metricName || 'Метрика')}</span>
                    </div>
