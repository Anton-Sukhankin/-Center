...
Карта хранит обзорную связь области с кодом, данными и пользовательским путем. Точный перечень файлов реализации, стилей и автоматических тестов принадлежит локальному README соответствующего компонента, feature или слоя; карта не дублирует эти перечни.
...
| Шапка проекта | `index.html`, `src/app/app.js` | `src/app/README.md`, `docs/context-behavior.md` | `window.activeContext`, `projectStructureData` | Пользователь видит выбранный контекст и атрибуты проекта; будущие разделы `Трекер задач`, `ГПР и Расписание`, `Процессные отчеты` видимы как отключенные пункты `Скоро` без перехода. |
...
| Центральный дашборд метрик | `src/features/metrics-dashboard/`, `src/app/app.js` | `src/features/metrics-dashboard/README.md`, `src/features/metrics-dashboard/sections/overview/README.md`, `src/features/metrics-dashboard/sections/procurement/README.md`, `docs/plans/metrics-real-data-integration-plan.md` | `window.metricsDashboardData`, внутренний `SCenterProcurementMetricsData`, `window.metricsDashboardViewModel`, `window.activeContext` | Пользователь выбирает проект или очередь и открывает `Метрики`; общая оболочка сохраняет контекст и единый `tabpanel`. `Общие` меняют сроки, этапы и бюджеты по `contextKey`; `Закупки` показывают согласованный demo-набор из 19 метрик и полную таблицу риска без локальных фильтров, не выводят alert над сводкой и сохраняют drawer процедуры. Для контекста не из источника закупочные значения не пересчитываются; ограничение остается в data-контракте без отдельной UI-пометки. |
...
| Карточка события | `src/components/event-card/`, `src/app/app.js` | `src/components/event-card/README.md`, `src/data/README.md` | `appData.getEventListViewModel()`, `taskData.getTaskCountForEvent()` | Пользователь читает событие, видит индикатор связанной задачи, закрепляет, открывает детали, исключает при активных фильтрах. |
| Детальная карточка события | `src/app/app.js`, `src/features/tasks/`, `src/features/bi/` | `src/app/README.md`, `src/components/event-card/README.md`, `docs/decision-actions.md`, `src/features/tasks/README.md`, `src/features/bi/README.md` | `appData.getEventDetailViewModel()`, `window.taskData`, `window.metricsData` | Пользователь раскрывает событие, видит расширенный состав данных и статичные карточки созданных задач, создаёт задачу, редактирует ее через кнопку с карандашом, удаляет через кнопку с корзиной либо открывает BI-график связанной метрики. |
| BI-график метрики | `src/features/bi/`, `src/app/app.js` | `src/features/bi/README.md`, `docs/entity-model.md` | Выбранный узел `window.metricsData`; демонстрационная помесячная серия feature | Пользователь нажимает связанную метрику в event drawer, изучает план/факт, включает прогноз и закрывает график без закрытия события. |
| Создание, редактирование и удаление задачи | `src/features/tasks/`, `src/app/app.js` | `src/features/tasks/README.md`, `docs/decision-actions.md`, `docs/evidence/tasks/` | `window.taskData`, исходное событие из `window.appData` | Пользователь нажимает кнопку «Создать задачу» в footer события, переходит в `task-create`, заполняет обязательные поля, выбирает ответственного из dropdown или команды проекта, создаёт задачу, позднее редактирует ее через icon-кнопку с карандашом или удаляет через icon-кнопку с корзиной. |
...
| Плавающая панель действий | `src/components/floating-action-bar/` | `src/components/floating-action-bar/README.md` | `window.toolbarState`, `window.filterState` | Пользователь видит режим фильтрации и может выйти из него; массовое создание задачи остается видимым отключенным действием `Скоро`. |
...
| Ассистент S.Center / ИИ-чат | `src/features/chat/`, `src/components/event-toolbar/` | `src/features/chat/README.md`, `src/components/event-toolbar/README.md`, `docs/plans/ai-chat-provider-actions-plan.md` | `window.activeContext`, `window.filterState`, `window.toolbarState`, `window.metricsData`, текущая экранная выборка событий | Пользователь открывает drawer `Ассистент S.Center` кнопкой `Задать вопрос` в правой зоне тулбара событий. |

`Сводка`, `Объекты` и `Шахматка` являются самостоятельными feature-поверхностями dropdown `Цифровая шахматка`. AI-чат является активным demo-сценарием; промышленный provider и actions остаются будущим этапом.
...
Состояния компонентов считаются частью карты интерфейса. Конкретное состояние и его переход обновляются в README соответствующего компонента или feature. `docs/component-states.md` обновляется только тогда, когда меняется общая семантика состояния, используемая несколькими областями.
