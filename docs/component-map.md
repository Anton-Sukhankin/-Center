...
| Сводка цифровой шахматки | `src/features/digital-chessboard-summary/`, `src/components/construction-table/`, `src/app/app.js`, `index.html` | `src/features/digital-chessboard-summary/README.md`, `src/components/construction-table/README.md`, `docs/reference/digital-chessboard-summary/README.md`, `docs/plans/digital-chessboard-summary-integration-plan.md`, `design-qa.md` | `window.digitalChessboardSummaryData`, `window.constructionObjectsData`, `window.objectsData`, `window.activeContext` | Пользователь выбирает `Сводка` в dropdown, меняет общий проект, изучает KPI, недельную динамику, проблемы и таблицу объектов, открывает архив, печатную форму и detail-drawer без потери состояния соседних экранов. |
