...
(function (window) {
    ...
    function buildConstructionValuesForQueue(queueId) {
        const seed = stableHash(queueId);
        const durationSeed = stableHash(`${queueId}:duration`);
        const volumeSeed = stableHash(`${queueId}:volume`);
        const durationPlan = 140 + (seed % 95);
        const durationDelta = (durationSeed % 31) - 8;
        const volumePlan = 8200 + (volumeSeed % 92) * 115;
        const volumeDelta = ((stableHash(`${queueId}:volume-delta`) % 33) - 12) * 85;

        return {
            duration: {
                plan: durationPlan,
                forecast: Math.max(1, durationPlan + durationDelta)
            },
            volume: {
                plan: volumePlan,
                forecast: Math.max(1, volumePlan + volumeDelta)
            }
        };
    }

    function getConstructionMetricsForContext(context) {
        const queueIds = context && context.queueIds && context.queueIds.length > 0
            ? context.queueIds
            : contextCatalog.map(item => item.queueId);

        const result = queueIds.reduce((acc, queueId) => {
            const values = buildConstructionValuesForQueue(queueId);
            acc.duration.plan += values.duration.plan;
            acc.duration.forecast += values.duration.forecast;
            acc.volume.plan += values.volume.plan;
            acc.volume.forecast += values.volume.forecast;
            return acc;
        }, {
            queueCount: queueIds.length,
            duration: { plan: 0, forecast: 0, delta: 0 },
            volume: { plan: 0, forecast: 0, delta: 0 }
        });

        result.duration.delta = result.duration.forecast - result.duration.plan;
        result.volume.delta = result.volume.forecast - result.volume.plan;
    ...
    window.appData = {
        ...
        getConstructionMetricsForContext,
        getEventSources,
        getPriorities,
