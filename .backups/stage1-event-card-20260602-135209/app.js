document.addEventListener('DOMContentLoaded', () => {
    ...
    let mockEventsTemplate = appData.cloneEvents();
    let mockEvents = appData.cloneEvents();
    window.metricsData = appData.cloneMetrics();

    // Projects List (Hierarchical)
    const businessUnits = [
        {
            id: 'bu-1',
        ...
        { id: 'bu-4', name: 'БИЗНЕС-ЮНИТ «САМОЛЕТ БАНК»', type: 'bu', children: [] },
        { id: 'bu-5', name: 'БИЗНЕС-ЮНИТ «САМОЛЕТ ПЛЮС»', type: 'bu', children: [] },
        { id: 'bu-6', name: 'БИЗНЕС-ЮНИТ «ГОСТЕПРИИМСТВО»', type: 'bu', children: [] }
    ];

    let activeEntity = { type: 'project', id: 'proj-nova', data: businessUnits[1].children[0], projectId: 'proj-nova' };
    ...
    function randomizeDataForContext() {
        mockEvents = JSON.parse(JSON.stringify(mockEventsTemplate));
    }

    window.setActiveEntity = function (type, id, projectId, entityData) {
        activeEntity.type = type;
        activeEntity.id = id;
        activeEntity.projectId = projectId;
    ...
    function renderEventFeed() {
        ...
        filterState.todayCount = todayFiltered.filter(e => !e.pinned).length;
        filterState.allCount = allFiltered.filter(e => !e.pinned).length;
        filterState.pinnedCount = mockEvents.filter(e => e.pinned && !e.excluded).length;

        const activePriorityFilter = filterState.priority || (filterState.priorityOnly ? 'high' : 'all');
        const hasFilters = filterState.period !== '' || filterState.sources.length > 0 || activePriorityFilter !== 'all' || selectedMetricFilters.length > 0 || filterState.searchQuery !== '';
        ...
        if (filterState.activeTab === 'today') {
            displayEvents = todayFiltered.filter(e => !e.pinned);
        } else if (filterState.activeTab === 'all') {
            displayEvents = allFiltered.filter(e => !e.pinned);
        } else if (filterState.activeTab === 'pinned') {
            displayEvents = mockEvents.filter(e => e.pinned && !e.excluded);
        }
        ...
        // NEW: Grouping by Metric Impact
        // Top Events = All events affecting metrics (positive or negative)
        // Other Events = Neutral status events
        const topImpactEvents = visibleEvents.filter(e => e.impactType === 'positive' || e.impactType === 'negative');
        const neutralStatusEvents = visibleEvents.filter(e => e.impactType === 'neutral' || !e.impactType);
        ...
        // Update counts
        document.getElementById('top-events-count').textContent = topImpactEvents.length;
        document.getElementById('other-events-count').textContent = neutralStatusEvents.length;
        ...
        topContainer.innerHTML = topImpactEvents.map(evt => renderEventCard(evt)).join('');

        const otherContainer = document.getElementById('other-events-container');
        otherContainer.innerHTML = neutralStatusEvents.map(evt => renderEventCard(evt)).join('');
    ...
    // High-Fidelity EventCard Component
    function renderEventCard(evt) {
        evt = appData.getEventListViewModel(evt, window.metricsData);
        const isPinned = evt.pinned ? 'pinned' : '';
        const priorityClass = `priority-${evt.priority}`;
        const impactTypeClass = (evt.impactType === 'negative') ? 'impact-negative' : 
                               (evt.impactType === 'positive' ? 'impact-positive' : 'impact-neutral');
        
        const isImpactCard = evt.impactType === 'positive' || evt.impactType === 'negative';

        return `
            <article class="event-card ${impactTypeClass} ${priorityClass} ${isPinned}" 
                     id="card-${evt.id}" 
                     data-event-id="${evt.id}"
                     onclick="openEventDrawer('${evt.id}')">
                
                <!-- Close button for exclusion -->
                <div class="selection-toggle-btn" 
                     onclick="toggleExclude('${evt.id}', event)" 
                     title="Исключить из выдачи">
                    <i data-lucide="x"></i>
                </div>

                <div class="event-card-top">
                    <div class="event-source-icon ${impactTypeClass}">
                        <i data-lucide="${evt.sourceIcon || 'activity'}"></i>
                    </div>
                    <div class="event-card-info">
                        ...
                        <div class="event-card-attributes-row">
                            <span class="event-source-text">${evt.sourceName || 'S.Center'}</span>
                            <span class="event-attributes-dot">•</span>
                            <span class="event-timestamp-text">${evt.dateText || '2026-03-11 08:30'}</span>
                        </div>
                    </div>
                    <div class="event-card-actions">
                        <button class="event-action-btn pin-toggle ${evt.pinned ? 'active' : ''}" 
                                onclick="togglePin('${evt.id}', event)" 
                                title="${evt.pinned ? 'Открепить' : 'Закрепить'}">
                            <i data-lucide="${evt.pinned ? 'pin-off' : 'pin'}" style="width:18px; height:18px;"></i>
                        </button>
                    </div>
                </div>

                <div class="event-card-desc">${evt.text}</div>

                ${isImpactCard ? `
                <div class="ec-footer-margin">
                    <footer class="ec-footer ${impactTypeClass}">
                        <div class="ec-footer-left">
                            <i data-lucide="${evt.impactType === 'negative' ? 'trending-down' : 'trending-up'}"></i>
                            <span>${evt.metricName || 'Метрика'}</span>
                        </div>
                        <div class="ec-footer-right">
                             <div class="ec-impact-value ${evt.impactType === 'negative' ? 'neg' : 'pos'}">${evt.impact}</div>
                        </div>
                    </footer>
                </div>
                ` : ''}
    ...
    window.toggleExclude = function(id, event) {
        ...
        if (evt) {
            ...
            // Task 4: Link to Metric Update
            if (evt.metricId && evt.impact) {
                const valNum = typeof evt.impactValue === 'number'
                    ? evt.impactValue
                    : parseFloat(evt.impact.replace(/[^\d.-]/g, ''));
                if (!isNaN(valNum)) {
                    // If we exclude it, we REVERSE the impact from the metrics (simulating removal of the problem)
                    const reverseImpact = evt.excluded ? -valNum : valNum;
                    if (window.updateMetrics) window.updateMetrics(reverseImpact, evt.metricId);
                }
            }
    ...
    function renderEventDrawer(evt) {
        ...
        // Sync header icon exactly with the event's sourceIcon
        const iconType = evt.sourceIcon || 'activity';
        ...
        titleNode.innerHTML = `
            ...
            <div class="drawer-header-main">
                <div class="drawer-header-title">${evt.title}</div>
                <div class="drawer-header-subtitle">
                    <span class="drawer-header-project-label">Проект:</span> 
                    <span class="drawer-header-project-value">Nova</span>
                </div>
            </div>
        `;
        ...
        drawerBody.innerHTML = `
            <div class="drawer-section drawer-section-flush">
                <div class="drawer-grid-params">
                    ...
                    <div class="param-row">
                        <div class="param-card">
                            <div class="param-card-info" style="gap:4px;">
                                <div class="param-card-label">Источник</div>
                                <div class="param-badge-container">
                                    <div class="param-badge purple">${evt.sourceName || 'S.Control'}</div>
                                </div>
                            </div>
                            <div class="param-card-icon"><i data-lucide="box"></i></div>
                        </div>
                        <div class="param-card">
                            <div class="param-card-info" style="gap:4px;">
                                <div class="param-card-label">Приоритет</div>
                                <div class="param-badge-container">
                                    <div class="param-badge ${evt.priority === 'high' ? 'volcano' : 'gray'}">${evt.priority === 'high' ? 'Высокий' : 'Низкий'}</div>
                                </div>
                            </div>
                            <div class="param-card-icon"><i data-lucide="alert-triangle"></i></div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="impact-card-top">
                    ...
                    <div class="impact-card-info">
                        ...
                        <div class="impact-card-trend ${evt.impactType === 'negative' ? 'neg' : 'pos'}">
                            ...
                            <span style="font-size:18px; font-weight:600;">${evt.impact || '-15 000 000 ₽'}</span>
                        </div>
                    </div>
                </div>
    ...
    window.updateMetrics = function(impactValue, metricId) {
        if (appData.applyMetricImpact(window.metricsData, metricId, impactValue)) {
            renderFinancialTree();
        }
    };
    ...
    function renderFinancialTree() {
        ...
        const deltaPrevMonthPct = active.deltaPrevMonth || -11.8; // Use data or mock
