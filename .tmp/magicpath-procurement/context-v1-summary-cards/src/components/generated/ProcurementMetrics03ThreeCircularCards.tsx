...
function SummaryMetricCard({
  ...
  return <article className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-[#e3e8ef] bg-white">
    <div className="flex min-h-[88px] items-start gap-2.5 px-3 py-3">
      <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-[10px] ${styles.icon}`}><Icon size={15} aria-hidden="true" /></span>
      <div className="min-w-0"><p className="min-h-8 text-[10px] font-medium leading-4 text-[#667085]">{label}</p><p className={`mt-1 whitespace-nowrap text-lg font-semibold leading-none ${styles.value}`}>{value}</p></div>
    </div>
    <div className="mt-auto min-h-9 border-t border-[#e3e8ef] bg-[#fbfcfe] px-3 py-2"><p className="truncate text-[9px] leading-4 text-[#667085]" title={note}>{note}</p></div>
  </article>;
}
