...
import { AlertTriangle, BarChart3, BriefcaseBusiness, Building2, CalendarDays, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, CircleAlert, Clock3, Database, FileBarChart2, Folder, FolderOpen, Grid2X2, HelpCircle, LayoutDashboard, Layers3, ListChecks, Menu, MessageCircleQuestion, PackageSearch, RotateCcw, Search, Send, ShieldAlert, SlidersHorizontal, TrendingUp, UserRound, UserRoundX, UsersRound, WalletCards, X } from 'lucide-react';
...
const risks: RiskRow[] = [{
  ...
  status: 'Критично',
  owner: 'Мария Громова',
  reason: 'Не подтверждён производственный слот поставщика',
  nextAction: 'Зафиксировать резервного поставщика до 12:00'
}, {
  id: 'T-15411',
  ...
  status: 'Внимание',
  owner: 'Алексей Панов',
  reason: 'Согласование технической части затянуто на 4 дня',
  nextAction: 'Эскалировать согласование главному инженеру'
}, {
  id: 'T-15562',
  ...
  status: 'Контроль',
  owner: 'Ирина Лаврова',
  reason: 'Срок стабилизирован, контроль цены до контракта',
  nextAction: 'Подтвердить финальную цену на комитете'
}, {
  id: 'T-15603',
  ...
  status: 'Внимание',
  owner: 'Олег Михайлов',
  reason: 'Повторный запрос коммерческих предложений',
  nextAction: 'Закрыть сбор предложений сегодня'
}, {
  id: 'T-15677',
  ...
  reserve: -2,
  status: 'Внимание',
  owner: 'Сергей Миронов',
  reason: 'Изменение спецификации после публикации',
  nextAction: 'Подписать новую спецификацию'
}, {
  id: 'T-15702',
  subject: 'Окна ПВХ',
  category: 'Работы',
  ...
  status: 'Критично',
  owner: 'Анна Орлова',
  reason: 'Единственный участник не подтвердил срок',
  nextAction: 'Запустить переговоры с альтернативой'
}, {
  id: 'T-15745',
  ...
  reason: 'Минимальный запас до требуемой даты',
  nextAction: 'Контрольный звонок поставщику 10.06'
}];
...
const workDrivers: Driver[] = [['Монолитные работы', '4 200', '4 460', '+260 млн ₽', '+6,2%', 92], ['Отделочные работы', '1 850', '2 020', '+170 млн ₽', '+9,2%', 70], ['Фасадные работы', '1 100', '1 010', '−90 млн ₽', '−8,2%', 48]];
const materialDrivers: Driver[] = [['Фасадные материалы', '820', '1 040', '+220 млн ₽', '+26,8%', 92], ['Лифтовое оборудование', '580', '700', '+120 млн ₽', '+20,7%', 68], ['Кабельно-проводниковая продукция', '330', '400', '+70 млн ₽', '+21,2%', 46]];
const efficiency = [['Средний срок проведения тендера', '18 дней', '14 дней', 82, 64, Clock3, true], ['Среднее количество участников на лот', '4,2', '3,1', 78, 58, UsersRound, false], ['Среднее количество лотов на тендер', '4,8', '3,0', 80, 50, Layers3, false], ['Доля лотов с одним участником', '18%', '10%', 72, 40, UserRoundX, true]] as const;
...
function SectionHeading({
  ...
  const tones = {
    blue: 'bg-[#eaf2ff] text-[#0b6bff]',
    red: 'bg-[#fff0f1] text-[#dc2626]',
    green: 'bg-[#ecf9f1] text-[#17834b]'
  };
...
export const GeneratedComponent = () => {
  ...
  return <div className="h-screen min-h-[760px] w-full overflow-hidden bg-[#f7f9fc] font-[Inter,Arial,sans-serif] text-[#111827]">
      ...
      <main className="min-w-0 flex-1 overflow-y-auto" aria-label="Метрики проекта">
        <div className="min-w-[920px] bg-white">
          <section className="flex items-center gap-4 px-6 pb-3 pt-4">
            <img src="https://storage.googleapis.com/storage.magicpath.ai/component-assets/438634486624837632/438638098767843328/2f1bcc5c24303ca25dcead382dd241950ae43a787ec6ed1fe031ee2e9f37a311.jpg" alt="Новый квартал" className="h-16 w-16 shrink-0 rounded-xl object-cover" />
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="grid gap-4 xl:grid-cols-[1.08fr_.92fr]">
            <section className="rounded-2xl border border-[#e3e8ef] bg-white p-5"><SectionHeading icon={WalletCards} title="Контрактация бюджета" caption="Бюджет закупок и покрытие договорами и открытыми процедурами" /><div className="mt-4 grid gap-4 md:grid-cols-[170px_1fr]"><div><p className="text-xs text-[#667085]">Общий бюджет закупок</p><p className="mt-1 text-[28px] font-normal leading-none">24,0 млрд ₽</p><p className="mt-3 text-[10px] text-[#667085]">Бюджет работ 14,1 · Бюджет материалов 9,9</p></div><div><div className="flex justify-between text-xs"><span className="font-medium">17,3 млрд ₽ законтрактовано · 72%</span><span className="text-[#667085]">24,0 млрд ₽</span></div><div className="mt-3 flex h-7 overflow-hidden rounded-lg bg-[#eef1f5]"><span className="grid bg-[#0b6bff] text-[10px] font-semibold text-white" style={{
                      width: '72%',
                      placeItems: 'center'
                    }}>72%</span><span className="grid bg-[#7c3aed] text-[10px] font-semibold text-white" style={{
                      width: '16%',
                      placeItems: 'center'
                    }}>16%</span><span className="grid text-[10px] font-semibold" style={{
                      width: '12%',
                      placeItems: 'center'
                    }}>12%</span></div><div className="mt-1.5 flex flex-wrap gap-3 text-[9px] text-[#667085]"><span>Договоры: 17,3 млрд ₽</span><span>Стоимость открытых тендеров: 3,9 млрд ₽ · 16%</span></div><div className="mt-3 grid grid-cols-2 gap-2">{[['Законтрактовано работ', '10,2 / 14,1'], ['Законтрактовано материалов', '7,1 / 9,9']].map(([label, value]) => <div key={label} className="rounded-xl bg-[#f8fafc] p-2.5"><div className="flex justify-between text-[10px]"><span>{label}</span><b>72%</b></div><div className="mt-2 h-1.5 rounded-full bg-[#e6eaf0]"><div className="h-full w-[72%] rounded-full bg-[#0b6bff]" /></div><p className="mt-1 text-[9px] text-[#667085]">{value} млрд ₽</p></div>)}</div></div></div></section>
            <section className="rounded-2xl border border-[#e3e8ef] bg-white p-5"><SectionHeading icon={BriefcaseBusiness} title="Эффективность закупочного процесса" caption="Факт относительно демонстрационной нормы" tone="green" /><div className="mt-4 grid gap-2 sm:grid-cols-2">{efficiency.map(([label, value, norm, fact, target, Icon, bad]) => <div key={label} className="flex items-center gap-2.5 rounded-xl bg-[#f8fafc] p-2.5"><span className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg ${bad ? 'bg-[#fff0f1] text-[#dc2626]' : 'bg-[#ecf9f1] text-[#17834b]'}`}><Icon size={14} /></span><div className="min-w-0 flex-1"><div className="flex justify-between gap-2 text-[10px]"><span className="truncate font-medium">{label}</span><b className={bad ? 'text-[#dc2626]' : 'text-[#17834b]'}>{value}</b></div><div className="relative mt-1.5 h-1.5 rounded-full bg-[#e6eaf0]"><span className={`absolute inset-y-0 left-0 rounded-full ${bad ? 'bg-[#dc2626]' : 'bg-[#17834b]'}`} style={{
                        width: `${fact}%`
                      }} /><span className="absolute -top-1 h-3.5 w-0.5 bg-[#111827]" style={{
                        left: `${target}%`
                      }} /></div><p className="mt-1 text-[9px] text-[#667085]">Норма: {norm}</p></div></div>)}</div></section>
          </div>

          <section className="rounded-2xl border border-[#e3e8ef] bg-white p-5"><div className="flex items-start justify-between gap-3"><SectionHeading icon={TrendingUp} title="Финансовый эффект закупок" caption="Отклонение стоимости договоров от бюджета" tone="red" /><div className="text-right"><p className="text-[10px] text-[#667085]">Сумма превышения / экономии бюджета</p><p className="text-lg font-medium text-[#dc2626]">+1,15 млрд ₽ · +4,8%</p></div></div><div className="mt-4 grid gap-4 xl:grid-cols-2">{[{ title: 'ТОП-3 работ с превышением бюджета', items: workDrivers }, { title: 'ТОП-3 материалов с превышением бюджета', items: materialDrivers }].map(group => <div key={group.title}><p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-[#667085]">{group.title}</p><div className="grid grid-cols-3 gap-2">{group.items.map(([name, budget, contract, delta, percent, width]) => <button key={name} type="button" className="rounded-xl border border-[#e3e8ef] p-3 text-left transition hover:bg-[#fffafa] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b6bff]"><div className="flex justify-between gap-2 text-xs"><span className="truncate font-semibold">{name}</span><span className={delta.startsWith('+') ? 'text-[#dc2626]' : 'text-[#17834b]'}>{percent}</span></div><div className="mt-3 h-2 rounded-full bg-[#edf0f4]"><div className={`h-full rounded-full ${delta.startsWith('+') ? 'bg-[#dc2626]' : 'bg-[#17834b]'}`} style={{
                    width: `${width}%`
                  }} /></div><div className="mt-2 flex justify-between text-[10px] text-[#667085]"><span>{budget} → {contract} млн ₽</span><b>{delta}</b></div></button>)}</div></div>)}</div></section>
    ...
    {drawer && <div className="fixed inset-0 z-40 bg-[#07182f]/20" onMouseDown={() => setDrawer(false)}><aside className="absolute inset-y-0 right-0 flex w-[380px] flex-col bg-white shadow-2xl" onMouseDown={e => e.stopPropagation()}><div className="flex items-center justify-between border-b border-[#e3e8ef] p-5"><div><p className="text-xs text-[#667085]">Контекст вопроса</p><h2 className="mt-1 font-semibold">Метрики → Закупки → {activeProject}</h2></div><button type="button" onClick={() => setDrawer(false)} className="grid h-9 w-9 place-items-center rounded-lg hover:bg-[#f5f7fa]" aria-label="Закрыть"><X size={18} /></button></div><div className="flex-1 p-5"><div className="rounded-xl bg-[#f5f8fc] p-4 text-xs leading-5 text-[#475467]">Вопрос будет дополнен текущими фильтрами: {period}, {category}, {status}.</div><label className="mt-5 block text-xs font-semibold" htmlFor="question">Ваш вопрос</label><textarea id="question" placeholder="Например: почему выросло отклонение по фасадным материалам?" className="mt-2 h-32 w-full resize-none rounded-xl border border-[#d7dee8] p-3 text-sm outline-none focus:border-[#0b6bff] focus:ring-2 focus:ring-[#eaf2ff]" /></div><div className="border-t border-[#e3e8ef] p-5"><button type="button" onClick={() => {
            setDrawer(false);
            showToast('Вопрос отправлен в демо-режиме');
