...
function CircularMetric({
  ...
  const tone = accent === 'violet' ? 'bg-[#f5f1ff] text-[#6d28d9]' : 'bg-[#eef5ff] text-[#0b6bff]';
  return <div className="flex min-w-0 items-center gap-3 rounded-xl border border-[#e3e8ef] bg-[#fbfcfe] p-3">
    <div className="relative grid h-[82px] w-[82px] shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(${color} 0 72%, #e7ebf0 72% 100%)` }} role="img" aria-label={`${label}: 72 процента`}>
      <span className="absolute inset-[9px] rounded-full bg-white" />
      <strong className="relative text-base font-semibold text-[#111827]">72%</strong>
    </div>
    <div className="min-w-0"><p className="text-[10px] font-medium leading-4 text-[#667085]">{label}</p><p className="mt-1 text-base font-semibold text-[#111827]">{value}</p><p className="mt-1 text-[9px] text-[#667085]">из бюджета {budget}</p><span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[8px] font-semibold ${tone}`}>Покрыто договорами</span></div>
  </div>;
}
      ...
      {items.map(item => {
        ...
        return <button key={item.name} type="button" className="group min-w-0 rounded-lg px-1 py-1 text-left transition hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b6bff]" aria-label={`${item.name}: бюджет ${item.budget} млн рублей, договор ${item.contract} млн рублей, отклонение ${item.delta}`}>
          <span className="flex h-[76px] items-end justify-center gap-1.5 border-b border-[#d7dee8] pb-1">
            <i className="w-4 rounded-t bg-[#a7b0bf] transition-colors group-hover:bg-[#8792a3]" style={{ height: `${Math.max(22, item.budget / max * 68)}px` }} />
            <i className={`w-4 rounded-t transition-colors ${overrun ? 'bg-[#ef4444] group-hover:bg-[#dc2626]' : 'bg-[#22a060] group-hover:bg-[#17834b]'}`} style={{ height: `${Math.max(22, item.contract / max * 68)}px` }} />
          </span>
...
export const ProcurementMetrics02CircularContracting = () => {
      ...
      <main className="min-w-0 flex-1 overflow-y-auto" aria-label="Метрики проекта">
        ...
        <div className="space-y-4 p-5">
          ...
          <section className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6" aria-label="Ключевые показатели закупок">
            {[['Общий бюджет закупок', '24,0 млрд ₽', 'Работы 14,1 · Материалы 9,9', 'text-[#111827]'], ['Законтрактовано бюджета', '17,3 млрд ₽', 'Доля законтрактованного бюджета 72%', 'text-[#111827]'], ['Отклонение стоимости договоров от бюджета', '+4,8%', 'Сумма превышения / экономии +1,15 млрд ₽', 'text-[#dc2626]'], ['Средний срок проведения тендера', '18 дней', 'Демо-норма 14 дней', 'text-[#dc2626]'], ['Среднее количество участников на лот', '4,2', 'Демо-норма 3,1', 'text-[#17834b]'], ['Закупки под риском', '7 тендеров', 'Демо-сумма 1,23 млрд ₽', 'text-[#dc2626]']].map(([label, value, note, tone]) => <article key={label} className="min-w-0 rounded-xl border border-[#e3e8ef] bg-white px-3 py-3"><p className="min-h-8 text-[10px] font-medium leading-4 text-[#667085]">{label}</p><p className={`mt-1 text-lg font-medium leading-none ${tone}`}>{value}</p><p className="mt-2 truncate text-[9px] text-[#667085]" title={note}>{note}</p></article>)}
          </section>

          <section className="rounded-2xl border border-[#e3e8ef] bg-white p-4" aria-label="Контрактация бюджета">
            <div className="flex flex-wrap items-start justify-between gap-3"><SectionTitle icon={WalletCards} title="Контрактация бюджета" note="Степень покрытия бюджета действующими договорами" /><div className="rounded-lg bg-[#f8fafc] px-3 py-2 text-right"><p className="text-[9px] text-[#667085]">Общий бюджет закупок</p><p className="mt-0.5 text-base font-semibold">24,0 млрд ₽</p></div></div>
            <div className="mt-3 grid gap-3 lg:grid-cols-[220px_minmax(0,1fr)]">
              <div className="flex items-center gap-4 rounded-xl border border-[#dbe5f2] bg-[#f8fbff] p-4">
                <div className="relative grid h-[116px] w-[116px] shrink-0 place-items-center rounded-full" style={{ background: 'conic-gradient(#0b6bff 0 72%, #7c3aed 72% 88%, #e7ebf0 88% 100%)' }} role="img" aria-label="Общий бюджет: 72 процента договоры, 16 процентов открытые тендеры, 12 процентов остаток">
                  <span className="absolute inset-[13px] rounded-full bg-white" />
                  <span className="relative text-center"><strong className="block text-2xl font-semibold leading-none">72%</strong><span className="mt-1 block text-[8px] text-[#667085]">законтрактовано</span></span>
                </div>
                <div className="min-w-0 space-y-2 text-[9px]"><div><span className="flex items-center gap-1.5 font-semibold text-[#0b6bff]"><i className="h-2.5 w-2.5 rounded-full bg-[#0b6bff]" />Договоры</span><p className="mt-0.5 pl-4 text-[#475467]">17,3 млрд ₽ · 72%</p></div><div><span className="flex items-center gap-1.5 font-semibold text-[#6d28d9]"><i className="h-2.5 w-2.5 rounded-full bg-[#7c3aed]" />Открытые тендеры</span><p className="mt-0.5 pl-4 text-[#475467]">3,9 млрд ₽ · 16%</p></div><div><span className="flex items-center gap-1.5 font-semibold text-[#667085]"><i className="h-2.5 w-2.5 rounded-full bg-[#d8dee7]" />Остаток</span><p className="mt-0.5 pl-4 text-[#475467]">2,8 млрд ₽ · 12%</p></div></div>
              </div>
              <div className="grid gap-3 sm:grid-cols-2"><CircularMetric label="Контрактация работ" value="10,2 млрд ₽" budget="14,1 млрд ₽" /><CircularMetric label="Контрактация материалов" value="7,1 млрд ₽" budget="9,9 млрд ₽" accent="violet" /></div>
            </div>
          </section>
