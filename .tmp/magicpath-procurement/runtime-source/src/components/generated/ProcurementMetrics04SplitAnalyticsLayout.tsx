...
function LinearMetric({
  ...
  const bar = accent === 'violet' ? 'bg-[#7c3aed]' : 'bg-[#0b6bff]';
  const soft = accent === 'violet' ? 'bg-[#f2edff] text-[#6d28d9]' : 'bg-[#eaf2ff] text-[#0b6bff]';
  return <article className="min-w-0 rounded-xl border border-[#e3e8ef] bg-[#fbfcfe] p-3">
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-medium leading-4 text-[#667085]">{label}</p><p className="mt-1 text-base font-medium leading-6 text-[#111827]">{value}</p></div><span className={`rounded-lg px-2.5 py-1 text-sm font-semibold ${soft}`}>72%</span></div>
    <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e7ebf0]" role="img" aria-label={`${label}: 72 процента`}><div className={`h-full w-[72%] rounded-full ${bar}`} /></div>
...
export const ProcurementMetrics04SplitAnalyticsLayout = () => {
      ...
      <main className="min-w-0 flex-1 overflow-y-auto" aria-label="Метрики проекта">
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="grid gap-3 min-[1080px]:grid-cols-12">
            <section className="rounded-2xl border border-[#e3e8ef] bg-white p-4 min-[1080px]:col-span-7" aria-label="Контрактация бюджета">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                ...
                <BudgetStructureCircle />
              </div>
              <div className="mt-3 grid gap-3 sm:grid-cols-2"><LinearMetric label="Контрактация работ" value="10,2 млрд ₽" budget="14,1 млрд ₽" /><LinearMetric label="Контрактация материалов" value="7,1 млрд ₽" budget="9,9 млрд ₽" accent="violet" /></div>
            </section>
            <section className="rounded-2xl border border-[#e3e8ef] bg-white p-4 min-[1080px]:col-span-5" aria-label="Финансовый эффект закупок">
          ...
          <section className="rounded-2xl border border-[#e3e8ef] bg-white p-4" aria-label="Эффективность закупочного процесса">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{efficiency.map(({
                label,
                value,
                norm,
                delta,
