...
(function (window) {
    ...
    const eventTemplates = rawEvents.concat(portfolioEvents).map(normalizeEvent).map(buildListPresentation);
    ...
    function getMetricsForContext(context, events) {
        ...
        contextEvents.forEach(event => {
            ...
            const value = typeof event.impactValue === 'number'
                ? event.impactValue
                : parseImpactValue(event.impact);
            if (value !== null) {
                applyMetricImpact(metrics, event.metricId, value);
            }
        });
        return metrics;
    }
