...
function BudgetLine({
  ...
  return <div className="rounded-xl bg-[#f8fafc] p-2.5">
    ...
    <div className="mt-1.5 h-1.5 rounded-full bg-[#e7ebf0]"><div className={`h-full rounded-full ${violet ? 'bg-[#7c3aed]' : 'bg-[#0b6bff]'}`} style={{
        width: `${percent}%`
      }} /></div>
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="grid gap-3 min-[1080px]:grid-cols-12">
            <section className="rounded-2xl border border-[#e3e8ef] bg-white p-4 min-[1080px]:col-span-7" aria-label="Финансовый эффект закупок">
              ...
              <div className="mt-3 grid gap-3 lg:grid-cols-2">{[{ title: 'ТОП-3 работ с превышением бюджета', items: workDrivers }, { title: 'ТОП-3 материалов с превышением бюджета', items: materialDrivers }].map(group => <div key={group.title}><p className="mb-1 text-[9px] font-semibold uppercase tracking-wide text-[#667085]">{group.title}</p><div className="space-y-1">{group.items.map(driver => {
                  const positive = driver.contract > driver.budget;
                  const max = Math.max(driver.budget, driver.contract) * 1.08;
                  return <button key={driver.name} type="button" className="w-full rounded-xl border border-transparent px-2 py-1.5 text-left transition hover:border-[#e3e8ef] hover:bg-[#fafbfc] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b6bff]"><span className="flex items-center justify-between gap-3 text-[10px]"><b className="truncate font-medium">{driver.name}</b><span className={`font-semibold ${positive ? 'text-[#dc2626]' : 'text-[#17834b]'}`}>{driver.delta} · {driver.percent}</span></span><span className="mt-1.5 grid grid-cols-[48px_minmax(0,1fr)] items-center gap-2 text-[8px] text-[#667085]"><span>Бюджет</span><span className="h-1 rounded-full bg-[#e8ecf1]"><i className="block h-full rounded-full bg-[#9aa5b5]" style={{
                          width: `${driver.budget / max * 100}%`
                        }} /></span></span><span className="mt-1 grid grid-cols-[48px_minmax(0,1fr)] items-center gap-2 text-[8px] text-[#667085]"><span>Договор</span><span className="h-1 rounded-full bg-[#e8ecf1]"><i className={`block h-full rounded-full ${positive ? 'bg-[#dc2626]' : 'bg-[#17834b]'}`} style={{
                          width: `${driver.contract / max * 100}%`
                        }} /></span></span></button>;
                })}</div></div>)}</div>
            </section>
            <section className="rounded-2xl border border-[#e3e8ef] bg-white p-4 min-[1080px]:col-span-5" aria-label="Эффективность закупочного процесса">
              <SectionTitle icon={BriefcaseBusiness} title="Эффективность закупочного процесса" note="Факт относительно демонстрационной нормы" tone="green" />
              <div className="mt-2 space-y-0.5">{efficiency.map(({
                  label,
                  value,
                  norm,
                  fact,
                  ...
                  Icon,
                  positive
                }) => <button key={label} type="button" className="flex w-full items-center gap-2 rounded-xl border border-transparent p-1.5 text-left transition hover:border-[#e3e8ef] hover:bg-[#fafbfc] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b6bff]"><span className={`grid h-6 w-6 place-items-center rounded-lg ${positive ? 'bg-[#ecf9f1] text-[#17834b]' : 'bg-[#fff0f1] text-[#dc2626]'}`}><Icon size={12} /></span><span className="min-w-0 flex-1"><span className="flex justify-between gap-2 text-[9px]"><b className="truncate font-medium">{label}</b><strong className={positive ? 'text-[#17834b]' : 'text-[#dc2626]'}>{value}</strong></span><span className="relative mt-1 block h-1 rounded-full bg-[#edf0f4]"><i className={`block h-full rounded-full ${positive ? 'bg-[#17834b]' : 'bg-[#dc2626]'}`} style={{
                        width: `${fact}%`
                      }} /><i className="absolute -top-1 h-3 w-0.5 bg-[#111827]" style={{
                        left: `${target}%`
                      }} /></span><span className="mt-0.5 block text-[8px] text-[#667085]">Норма: {norm}</span></span></button>)}</div>
            </section>
          </div>
